...
class TestCreate:
    async def test_creates_project_with_valid_owner(self, service, mock_repo, mock_user_repo):
        ...
        mock_repo.create.assert_called_once_with(
            name="My Project", description="A test project", owner_id="u1", status="active"
        )

    async def test_raises_not_found_when_owner_missing(self, service, mock_user_repo):
        mock_user_repo.find_by_id.return_value = None
        with pytest.raises(NotFoundError):
            await service.create("My Project", "A test project", "u999")
...
class TestGetById:
    ...
    async def test_raises_not_found_when_missing(self, service, mock_repo):
        ...
        with pytest.raises(NotFoundError):
            await service.get_by_id("p999")


class TestListByOwner:
    async def test_returns_projects_for_owner(self, service, mock_repo):
        mock_repo.find_by_owner_id.return_value = [_make_project(), _make_project(id="p2", name="Second")]
        projects = await service.list_by_owner("u1")
        assert len(projects) == 2

    async def test_returns_empty_list_when_no_projects(self, service, mock_repo):
...
class TestUpdate:
    async def test_updates_name(self, service, mock_repo):
        ...
        result = await service.update("p1", name="Updated Name")
    ...
    async def test_updates_description(self, service, mock_repo):
        ...
        mock_repo.find_by_id.return_value = project
        mock_repo.update.return_value = project
        result = await service.update("p1", description="New desc")
        assert result.description == "New desc"

    async def test_raises_not_found_when_missing(self, service, mock_repo):
        mock_repo.find_by_id.return_value = None
        with pytest.raises(NotFoundError):
            await service.update("p999", name="Nope")
...
class TestArchive:
    ...
    async def test_raises_not_found_when_missing(self, service, mock_repo):
        mock_repo.find_by_id.return_value = None
        with pytest.raises(NotFoundError):
            await service.archive("p999")

    async def test_raises_validation_error_when_already_archived(self, service, mock_repo):
