...
describe('AuthService', () => {
  ...
  describe('logout', () => {
    it('deletes session by token', async () => {
      ...
      expect(db.sessions.deleteByToken).toHaveBeenCalledWith('some-token');
    });
  });

  describe('validateSession', () => {
    it('returns user for valid session', async () => {
      vi.mocked(db.sessions.findByToken).mockResolvedValue({ userId: '1', token: 't' } as any);
      vi.mocked(db.users.findById).mockResolvedValue({ id: '1', email: 'a@b.com' } as any);
      const user = await service.validateSession('t');
  ...
  describe('changePassword', () => {
    ...
    it('throws if user not found', async () => {
      ...
      await expect(service.changePassword('999', 'old', 'new')).rejects.toThrow('User not found');
    });
  });
